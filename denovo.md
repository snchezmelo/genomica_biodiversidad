...
: -b = ruta a un archivo que contiene el listado de los barcodes
: -o = ruta donde poner los archivos procesados
...
: -e [enz], --renz_1 [enz] = la enzima de restrición utilizada
...
: -f = ruta del archivo de entrada con el nombre del archivo
: -o = ruta donde escribir los resultados
: -i = un ID de numérico único para cada muestra
...
: --model_type = 'snp' (predeterminado), 'bounded', o 'fixed'
...
: -p = habilitar la ejecución paralela con número de subprocesos (usaremos -p 8)
: -n número de discrepancias permitidas entre los loci putativos cuando se crea el catálogo (predeterminado 1)
...
: -c,--catatlog = path to the catalog
...
: -P,--in-dir = directorio de entrada que tenga los archivos generados por *ustacks* y *sstacks*
: -M,--popmap — ruta a un mapa de población (lo mismo que *stacks*)
: -t — habilitar la ejecución paralela con número de subprocesos (predeterminado 1)
...
: -P = directorio de entrada que contiene archivos '* .matches.bam' creados por el pipeline *de novo* de Stacks
...
: -P,--in_path = directorio de entrada que tenga los archivos generados por Stacks
: -O,--out_path = ruta a un directorio donde escribir los archivos de salida. (predeterminado al valor de -P.)
: -M,--popmap = ruta a un mapa de población (lo mismo que se usó antes)
: -t,--threads = habilitar la ejecución paralela con número de subprocesos (predeterminado 1)
: -p,--min-populations [int] = número mínimo de poblaciones en las que debe estar presente un locus para ser procesado
: -r,--min-samples-per-pop [float] = porcentaje mínimo de individuos en una población requerido para procesar un locus para esa población
: -R,--min-samples-overall [float] = porcentaje mínimo de individuos en poblaciones requerido para procesar un locus
: --min-mac [int] = especificar el número mínimo de muestras que un alelo tiene que estar presente para procesar un SNP (aplicado a la metapoblación).
: --max-obs-het [float] = especificar la heterocigosidad máxima observada requerida para procesar un sitio de nucleótidos en un locus (aplicado a la metapoblación)
...
Para este conjunto de datos, vamos correr *populations* dos veces. Uno usando todos los SNPs para calcular estadísticas sumarias y otro para generar los archivos para diferentes programas usando solamente un SNP aleatorio por loci. Crear una carpeta para cada vez que vamos correr *populations*.
